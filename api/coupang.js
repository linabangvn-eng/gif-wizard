...
function generateHmac(method, url, secretKey, accessKey) {
    ...
    const datetime = d.getUTCFullYear().toString().substring(2, 4) + pad(d.getUTCMonth() + 1) + pad(d.getUTCDate()) + 'T' + pad(d.getUTCHours()) + pad(d.getUTCMinutes()) + pad(d.getUTCSeconds()) + 'Z';
...
module.exports = (req, res) => {
    ...
    try {
        ...
        const options = { hostname: 'api-gateway.coupang.com', port: 443, path: endpoint, method: method, headers: { 'Authorization': authHeader, 'Content-Type': 'application/json' } };
        const coupangReq = https.request(options, (coupangRes) => {
            let data = '';
            coupangRes.on('data', (chunk) => { data += chunk; });
            coupangRes.on('end', () => { try { res.status(200).json(JSON.parse(data)); } catch (err) { res.status(500).json({ error: 'Parse error' }); } });
        });
