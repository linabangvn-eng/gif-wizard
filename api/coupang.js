...
module.exports = (req, res) => {
    ...
    // CORS Header setting for local testing or cross-origin
        ...
        const coupangReq = https.request(options, (coupangRes) => {
            ...
            coupangRes.on('end', () => {
                try {
                    ...
                    res.status(500).json({ error: 'Failed to parse Coupang JSON.' });
                }
            });
        });

        coupangReq.on('error', (error) => {
            res.status(500).json({ error: 'Coupang API Request Error: ' + error.message });
        });
